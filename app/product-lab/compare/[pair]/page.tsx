...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!parsed) return { title: "Comparison — Sleep Layers Product Lab" };
  const [a, b] = parsed;
  const pa = findProductBySlug(a);
  const pb = findProductBySlug(b);
  if (!pa || !pb) return { title: "Comparison — Sleep Layers Product Lab" };
  return {
    title: `${pa.name} vs ${pb.name} — Sleep Layers Product Lab`,
...
export default async function ComparePage({ params }: Props) {
  ...
  return (
    <article className="mx-auto max-w-5xl px-4 py-16 lg:px-8 lg:py-24">
      ...
      <h1 className="mt-4 font-serif text-3xl font-medium leading-tight tracking-tight text-navy md:text-4xl">
        {a.name} vs {b.name}
      </h1>
      ...
      <section className="mt-14">
        ...
        <div className="mt-5 overflow-hidden rounded-2xl border border-hairline">
          <table className="w-full text-left text-[13px]">
            <thead className="bg-paper-2 text-[10px] uppercase tracking-wider text-ink-2">
              <tr>
            ...
            <tbody className="divide-y divide-hairline bg-paper">
              {specRows.map((row) => (
                <tr key={row.label}>
                  ...
                  <td className="px-4 py-3 text-ink">{row.a}</td>
                  <td className="px-4 py-3 text-ink">{row.b}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <p className="mt-14 text-center text-[12px] text-ink-2">
        See each product's full review:{" "}
        <Link
          href={`/product-lab/${a.slug}`}
          className="text-navy underline decoration-amber underline-offset-2 hover:text-amber"
        >
          {a.name}
        </Link>
        {" · "}
...
function CompareCard({
  ...
  product: ReturnType<typeof findProductBySlug> & {};
}) {
  return (
    <Link
      href={`/product-lab/${product.slug}`}
      className="block rounded-2xl border border-hairline bg-paper p-6 transition-all hover:border-amber"
    >
